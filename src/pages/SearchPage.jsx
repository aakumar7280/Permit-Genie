...
import { ArrowUp, Sparkles, Bookmark } from 'lucide-react';
import Header from '../components/Header';
import { permitsAPI, savedPermitsAPI } from '../services/api';

const SearchPage = () => {
  const navigate = useNavigate();
  const [query, setQuery] = useState('');
  const [loading, setLoading] = useState(false);
  const [messages, setMessages] = useState([]); // Chat history for display
  const [chatHistory, setChatHistory] = useState([]); // Sent to backend
  const [savedSet, setSavedSet] = useState(new Set()); // Track saved permit IDs
  ...
  // Load user's saved permits on mount so toggle state is accurate
  useEffect(() => {
    const loadSaved = async () => {
      try {
        const token = localStorage.getItem('token');
        if (!token) return;
        const res = await savedPermitsAPI.getSavedPermits();
        if (res.success) {
          setSavedSet(new Set(res.savedPermits.map(s => s.permitId)));
        }
      } catch (_) { /* not logged in or network error — ignore */ }
    };
    loadSaved();
  }, []);
  ...
  const handleSend = async (text = query) => {
    ...
    // Build updated chat history for the backend
  ...
  return (
    <div className="min-h-screen relative overflow-hidden" style={{ backgroundColor: '#191919' }}>
      <Header />
      ...
      {/* Main Content */}
      <div className="flex flex-col relative z-10" style={{ height: 'calc(100vh - 0px)', paddingTop: '80px' }}>
                              ...
                              {msg.permits.map((permit) => (
                                ...
                                >
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    <button
                                      onClick={() => navigate(`/apply/${permit.id}`, { state: { permit } })}
                                      className="px-4 py-1.5 text-white rounded-lg transition-colors duration-200 text-xs font-medium hover:opacity-90"
                                    ...
                                    <button
                                      onClick={async () => {
                                        try {
                                          if (savedSet.has(permit.id)) {
                                            await savedPermitsAPI.unsavePermit(permit.id);
                                            setSavedSet(prev => { const n = new Set(prev); n.delete(permit.id); return n; });
                                          } else {
                                            await savedPermitsAPI.savePermit(permit.id);
                                            setSavedSet(prev => new Set(prev).add(permit.id));
                                          }
                                        } catch (e) { console.error('Save toggle failed:', e); }
                                      }}
                                      className="flex items-center gap-1 px-3 py-1.5 rounded-lg transition-colors duration-200 text-xs font-medium"
                                      style={{
                                        backgroundColor: savedSet.has(permit.id) ? 'rgba(241,80,37,0.15)' : '#2d2d2d',
                                        border: savedSet.has(permit.id) ? '1px solid #F15025' : '1px solid #444',
                                        color: savedSet.has(permit.id) ? '#F15025' : '#CED0CE',
                                      }}
                                      title={savedSet.has(permit.id) ? 'Unsave permit' : 'Save for later'}
                                    >
                                      <Bookmark size={12} fill={savedSet.has(permit.id) ? '#F15025' : 'none'} />
                                      {savedSet.has(permit.id) ? 'Saved' : 'Save'}
                                    </button>
                                  </div>
                                </div>
                              ))}
